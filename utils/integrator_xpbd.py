...
def apply_joint_torques(
    ...
    tid = wp.tid()
    type = joint_type[tid]
    if type == fs5Model.JOINT_FIXED:
        return
    if type == fs5Model.JOINT_FREE:
        return
    if type == fs5Model.JOINT_DISTANCE:
        return
    if type == fs5Model.JOINT_BALL:
        return
    ...
    if type == fs5Model.JOINT_REVOLUTE:
        axis = joint_axis[axis_start]
        act = joint_act[qd_start]
        a_p = wp.transform_vector(X_wp, axis)
        t_total += act * a_p
    elif type == fs5Model.JOINT_PRISMATIC:
        axis = joint_axis[axis_start]
        act = joint_act[qd_start]
        a_p = wp.transform_vector(X_wp, axis)
        f_total += act * a_p
    elif type == fs5Model.JOINT_COMPOUND:
    ...
    elif type == fs5Model.JOINT_UNIVERSAL:
        # q_off = wp.transform_get_rotation(X_cj)
        # q_pc = wp.quat_inverse(q_off)*wp.quat_inverse(q_p)*q_c*q_off
...
def solve_body_joints(
    ...
    if joint_enabled[tid] == 0 or type == fs5Model.JOINT_FREE:
        return
    ...
    if (
        type == fs5Model.JOINT_FIXED
        or type == fs5Model.JOINT_PRISMATIC
        or type == fs5Model.JOINT_REVOLUTE
        or type == fs5Model.JOINT_UNIVERSAL
        or type == fs5Model.JOINT_COMPOUND
        or type == fs5Model.JOINT_D6
    ):
        # handle angular constraints

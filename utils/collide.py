...
import warp as wp
from .fs5Model import PARTICLE_FLAG_ACTIVE, ModelShapeGeometry
...
def create_soft_contacts(
    ...
    if geo_type == fs5Model.GEO_PLANE:
...
def count_contact_points(
    ...
    if shape_b == -1:
        ...
        # ground plane
        actual_type_b = fs5Model.GEO_PLANE
    else:
    ...
    num_contacts = 0
    if actual_type_a == fs5Model.GEO_SPHERE:
        num_contacts = 1
    elif actual_type_a == fs5Model.GEO_CAPSULE:
        if actual_type_b == fs5Model.GEO_PLANE:
        ...
        elif actual_type_b == fs5Model.GEO_MESH:
            num_contacts_a = 2
            ...
            num_contacts = 2
    elif actual_type_a == fs5Model.GEO_BOX:
        if actual_type_b == fs5Model.GEO_BOX:
            num_contacts = 24
        elif actual_type_b == fs5Model.GEO_MESH:
            num_contacts_a = 8
            mesh_b = wp.mesh_get(geo.source[actual_shape_b])
            num_contacts_b = mesh_b.points.shape[0]
            num_contacts = num_contacts_a + num_contacts_b
        elif actual_type_b == fs5Model.GEO_PLANE:
            if geo.scale[actual_shape_b][0] == 0.0 and geo.scale[actual_shape_b][1] == 0.0:
            ...
            num_contacts = 8
    elif actual_type_a == fs5Model.GEO_MESH:
        mesh_a = wp.mesh_get(geo.source[actual_shape_a])
        num_contacts_a = mesh_a.points.shape[0]
        if actual_type_b == fs5Model.GEO_MESH:
            mesh_b = wp.mesh_get(geo.source[actual_shape_b])
            num_contacts_b = mesh_b.points.shape[0]
        else:
            num_contacts_b = 0
        num_contacts = num_contacts_a + num_contacts_b
    elif actual_type_a == fs5Model.GEO_PLANE:
...
def broadphase_collision_pairs(
    ...
    if actual_type_b == fs5Model.GEO_PLANE:
        if actual_type_a == fs5Model.GEO_PLANE:
            return
...
def handle_contact_pairs(
    ...
    if geo_type_a == fs5Model.GEO_SPHERE:
        p_a_world = wp.transform_get_translation(X_ws_a)
        if geo_type_b == fs5Model.GEO_SPHERE:
            p_b_world = wp.transform_get_translation(X_ws_b)
        elif geo_type_b == fs5Model.GEO_BOX:
        ...
        elif geo_type_b == fs5Model.GEO_CAPSULE:
        ...
        elif geo_type_b == fs5Model.GEO_MESH:
            ...
            if res:
                ...
                return
        elif geo_type_b == fs5Model.GEO_PLANE:
    ...
    elif geo_type_a == fs5Model.GEO_BOX and geo_type_b == fs5Model.GEO_BOX:
    ...
    elif geo_type_a == fs5Model.GEO_BOX and geo_type_b == fs5Model.GEO_CAPSULE:
        half_height_b = geo_scale_b[1]
        # capsule B
    ...
    elif geo_type_a == fs5Model.GEO_BOX and geo_type_b == fs5Model.GEO_PLANE:
    ...
    elif geo_type_a == fs5Model.GEO_CAPSULE and geo_type_b == fs5Model.GEO_CAPSULE:
    ...
    elif geo_type_a == fs5Model.GEO_CAPSULE and geo_type_b == fs5Model.GEO_MESH:
    ...
    elif geo_type_a == fs5Model.GEO_MESH and geo_type_b == fs5Model.GEO_CAPSULE:
        # vertex-based contact
    ...
    elif geo_type_a == fs5Model.GEO_CAPSULE and geo_type_b == fs5Model.GEO_PLANE:
        plane_width = geo_scale_b[0]
        plane_length = geo_scale_b[1]
        if point_id < 2:
    ...
    elif geo_type_a == fs5Model.GEO_MESH and geo_type_b == fs5Model.GEO_BOX:
        # vertex-based contact
        mesh = wp.mesh_get(geo.source[shape_a])
        body_a_pos = wp.cw_mul(mesh.points[point_id], geo_scale_a)
        p_a_world = wp.transform_point(X_ws_a, body_a_pos)
    ...
    elif geo_type_a == fs5Model.GEO_BOX and geo_type_b == fs5Model.GEO_MESH:
        # vertex-based contact
    ...
    elif geo_type_a == fs5Model.GEO_MESH and geo_type_b == fs5Model.GEO_MESH:
        # vertex-based contact
        mesh = wp.mesh_get(geo.source[shape_a])
        mesh_b = geo.source[shape_b]
    ...
    elif geo_type_a == fs5Model.GEO_MESH and geo_type_b == fs5Model.GEO_PLANE:
        # vertex-based contact
        mesh = wp.mesh_get(geo.source[shape_a])
        body_a_pos = wp.cw_mul(mesh.points[point_id], geo_scale_a)
        p_a_world = wp.transform_point(X_ws_a, body_a_pos)
